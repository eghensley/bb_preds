...
def check_lr(lr, x, y, scale):
    ...
    for tree in [75, 100, 125]:
        ...
        score = cross_val_score(test, x, y, scoring = 'neg_mean_squared_error' ,cv = KFold(n_splits = 10, random_state = 86))
        scores.append(np.mean(score))
...
def drop_lr(l_drop, trees):
    ...
    for trial in np.linspace(1.5, 8.5, 15):
        ...
        lr_score = cross_val_score(model_lr, x_data[feat_sigs[:features]], y_data, scoring = 'neg_mean_squared_error' ,cv = KFold(n_splits = 10, random_state = 151))
...
def execute(sa, od, X_data = None, Y_data = None):
    ...
    global scale
    f = open(os.path.join(output_folder, '%s-%s-lightgbm.txt'%(sa, od)), 'w')
    ...
    tune_score = cross_val_score(tune_model, x_data[feat_sigs[:features]], y_data, scoring = 'neg_mean_squared_error' ,cv = KFold(n_splits = 10, random_state = 86))
