...
def test_scaler(x, y):
    ...
    for scale in [StandardScaler(), MinMaxScaler(), RobustScaler()]:
        pipe = Pipeline([('scale',scale), ('clf',LinearSVC(random_state = 1108, epsilon=0))])
...
def sample_loss_n_feats(parameters):
    ...
    model = Pipeline([('scale',scale),  ('clf',LinearSVC(random_state = 1108, C = C_, epsilon=0))])
...
def sample_loss_c(parameters):
    c = 10**parameters[0]
    model = Pipeline([('scale',scale), ('clf',LinearSVC(random_state = 1108, C = c, epsilon=0))])
