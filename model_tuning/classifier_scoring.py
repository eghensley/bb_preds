...
for sort in ['ou']: #'winner', 'line', 
    ...
    for kind in ['raw']: #'+pts', 
        ...
        for model_name, model_details in saved_models.stored_models[sort][kind].items():
            ...
            scores = cross_validate(model_details['model'], all_x_data[sort][kind][model_details['features']], all_y_data[sort][kind], cv = StratifiedKFold(n_splits = 20, random_state = 1108), n_jobs = -1, scoring = ['neg_log_loss', 'accuracy'], verbose = 1)
            print('...Scored %s Values'%(model_name))
            f = open(os.path.join(output_folder, 'classification_scoring.txt'), 'a')
            f.write('%s, %s, %s\n' % (model_name, np.mean(scores['test_accuracy']), np.mean(scores['test_neg_log_loss'])))
            f.close()
