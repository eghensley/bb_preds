...
x_vals = 'predictive'
y_val = '+pts'
x_data = pull_data.score(update_dbs.mysql_client())
x_cols = list(x_data)
x_cols.remove(y_val)
y_data = x_data[y_val] 
...
def test_scaler(x, y):
    ...
    for scale in [StandardScaler(), MinMaxScaler(), RobustScaler()]:
        pipe = Pipeline([('scale',scale), ('clf',KerasRegressor(build_fn=baseline_model, epochs=50, batch_size=64, verbose=1))])
...
estimators.append(('mlp', KerasRegressor(build_fn=baseline_model, epochs=50, batch_size=64, verbose=1)))
