...
stored_models = {
        'offense':{
            'pace': {
                ...
                'model': Pipeline([('scale',StandardScaler()), ('reg',Lasso(random_state = 1108, alpha = 0.019012104600893226))]),
            },    
            'ppp': {
                'features': ['linsvm_all', 'ridge_all', 'lightgbm_target', 'ridge_target', 'lightgbm_team', 'linsvm_team', 'rest', 'lasso_target', 'linsvm_target'],
                'model': Pipeline([('scale',RobustScaler()), ('reg',LinearSVR(random_state = 1108, C = 0.215329117725, epsilon=0))]),
            },                 
        },
        'defense':{
            'pace': {
                'features': ['lasso_possessions', 'lightgbm_possessions', 'ridge_possessions'],
                'model': Pipeline([('scale',MinMaxScaler()), ('reg',LinearSVR(random_state = 1108, C = 0.6652011887133216, epsilon=0))]),
            },  
            'ppp': {
                'features': ['lightgbm_all', 'ridge_all', 'lasso_target', 'lightgbm_target', 'lasso_team', 'lightgbm_team', 'linsvm_team', 'ridge_team', 'rest'],
                'model': Pipeline([('scale',StandardScaler()), ('reg',Ridge(random_state = 1108, solver = 'lsqr', alpha = 0.00101115979472))]),
            },  
        },
        'predictive':{
                    '+pts': {
                        'features': ['lightgbm_allow_all', 'lightgbm_for_target', 'lightgbm_for_possessions', 'lasso_for_possessions', 'ridge_for_all', 'linsvm_for_all', 'linsvm_for_team', 'lightgbm_for_team', 'ridge_for_target', 'lightgbm_allowed_target', 'ridge_allow_possessions', 'lightgbm_allow_possessions', 'lasso_allow_possessions', 'ridge_allow_team', 'linsvm_allow_team', 'lightgbm_allow_team', 'lasso_allow_team', 'ridge_allow_all', 'lasso_allowed_target', 'lasso_for_target'],
                        'model': Pipeline([('scale',RobustScaler()), ('reg',Ridge(random_state = 1108, solver = 'lsqr', alpha = 0.859276035795))]),
                        },    
        },
        'winner':{
                '+pts':{
                    'log': {
                        ...
                        'model': Pipeline([('scale',StandardScaler()), ('clf', LogisticRegression(random_state = 1108, C = 1000, solver = "liblinear"))]),
                ...
                'raw':{
                    ...
                    'linsvc': {
                        ...
                        'model': Pipeline([('scale',StandardScaler()), ('clf',SVC(random_state = 1108, C = 0.0207173498763, kernel = 'linear', probability = True))]),
                    ...
                    'lightgbc': {
                        ...
                        'model': Pipeline([('scale',StandardScaler()), ('clf', lgb.LGBMClassifier(random_state = 1108, n_estimators = 100, colsample_bytree = 0.642120736080607, min_child_samples = 116, num_leaves = 12, subsample = 0.897114330960264, max_bin = 1021, learning_rate = 0.08))]),
        ...
        'ou':{
                '+pts':{      
                    ...
                    'linsvc': {
                        ...
                        'model': Pipeline([('scale',MinMaxScaler()), ('clf',SVC(random_state = 1108, C =  0.15817437173, kernel = 'linear', probability = True))]),
                    ...
                    'lightgbc': {
                        ...
                        'model': Pipeline([('scale',StandardScaler()), ('clf', lgb.LGBMClassifier(random_state = 1108, n_estimators = 100, colsample_bytree = 0.925424645171526, min_child_samples = 63, num_leaves = 159, subsample = 0.417196512684593, max_bin = 1011, learning_rate = 0.01))]),
        ...
        'line':{
                '+pts':{
                    ...
                    'linsvc': {
                        ...
                        'model': Pipeline([('scale',StandardScaler()), ('clf',SVC(random_state = 1108, C =  4.87778133978, kernel = 'linear', probability = True))]),
                    ...
                    'lightgbc': {
                        ...
                        'model': Pipeline([('scale',RobustScaler()), ('clf', lgb.LGBMClassifier(random_state = 1108, n_estimators = 225, colsample_bytree =0.985094185628027, min_child_samples = 13, num_leaves = 15, subsample = 0.721806428713343, max_bin = 1110, learning_rate = 0.03))]),
        ...
        'result':{
                'line':{
                    'ridge': {
                        ...
                        'model': Pipeline([('scale',MinMaxScaler()), ('reg',Ridge(random_state = 1108, solver = 'sag', alpha = 584.39992591))]),
                        },
                    'lasso': {
                        'features': ['ha', '10_game_avg', 'streak'],
                        'model': Pipeline([('scale',StandardScaler()), ('reg',Lasso(random_state = 1108, alpha = 0.0011089952827, max_iter = 2000))]),
                        },
                    'lightgbm': {
                        'features': ['10_game_avg', 'streak', 'ha', '50_game_avg'],
                        'model': Pipeline([('scale',StandardScaler()), ('reg', lgb.LGBMRegressor(random_state = 1108, n_estimators = 1000, colsample_bytree = 0.980189782695348, min_child_samples = 189, num_leaves = 14, subsample = 0.791883150188403, max_bin = 1988, learning_rate = 0.00140625))]),
                        }
                },
                'ou': {
                    'ridge': {
                        'features': ['10_game_avg', '15_game_avg', '50_game_avg', '5_game_avg', 'streak', '30_game_avg'],
                        'model': Pipeline([('scale',MinMaxScaler()), ('reg',Ridge(random_state = 1108, solver = 'lsqr', alpha = 737.480281596))]),
                        },
                    'lasso': {
                        'features': ['10_game_avg', '15_game_avg', '50_game_avg', '5_game_avg', 'streak', '30_game_avg'],
                        'model': Pipeline([('scale',StandardScaler()), ('reg',Lasso(random_state = 1108, alpha = 0.00171909758817))]),
                        },
                    'lightgbm': {
                        'features': ['3_game_avg', '15_game_avg', '10_game_avg', '5_game_avg', 'streak', '50_game_avg', '30_game_avg'],
                        'model': Pipeline([('scale',StandardScaler()), ('reg', lgb.LGBMRegressor(random_state = 1108, n_estimators = 300, colsample_bytree = 0.952264164385702, min_child_samples = 12, num_leaves = 36, subsample = 0.64633747168907, max_bin = 1138, learning_rate = 0.015))]),                
                }
            },
        },
        'pts_allowed': {
                'all': {
                    'ridge': {
                        ...
                        'model': Pipeline([('scale',MinMaxScaler()), ('reg',Ridge(random_state = 1108, solver = 'sparse_cg', alpha = 0.89393381144))]),
                        },
                    'lightgbm': {
                        ...
                        'model': Pipeline([('scale',StandardScaler()), ('reg', lgb.LGBMRegressor(random_state = 1108, n_estimators = 400, colsample_bytree = 0.796359005305649, min_child_samples = 198, num_leaves = 13, subsample = 0.65344498465166, max_bin = 1953, learning_rate = 0.03))]),
                        }
                    },
                'target':{
                    'lightgbm': {
                        ...
                        'model': Pipeline([('scale',MinMaxScaler()), ('reg', lgb.LGBMRegressor(random_state = 1108, n_estimators = 900, colsample_bytree = 0.956416580127077, min_child_samples = 4, num_leaves = 61, subsample = 0.631441132669909, max_bin = 1974, learning_rate = 0.00375))]),
                        },                         
                    'lasso': {
                        ...
                        'model': Pipeline([('scale',StandardScaler()), ('reg',Lasso(random_state = 1108, alpha = 0.001, max_iter = 2000))]),
                        },
                    },
                'possessions': {
                    'lightgbm': {
                        ...
                        'model': Pipeline([('scale',StandardScaler()), ('reg', lgb.LGBMRegressor(random_state = 1108, n_estimators = 4050, colsample_bytree = 0.947978555103353, min_child_samples = 4, num_leaves = 20, subsample = 0.417125690980936, max_bin = 1114, learning_rate = 0.00125))]),
                        },
                    'ridge': {
                        ...
                        'model': Pipeline([('scale',MinMaxScaler()), ('reg',Ridge(random_state = 1108, solver = 'lsqr', alpha = 4.095337845324041))]),
                        }, 
                    'lasso': {
                        'features': ['expected_poss_pg_allowed', 'pregame_poss_pg_allowed', '5_game_avg_25_g_Tweight_allow_possessions-per-game', '5_game_avg_10_g_Tweight_allow_possessions-per-game', '25_g_HAspread_allow_possessions-per-game', '1_game_avg_50_g_HAweight_allow_possessions-per-game', '1_game_avg_10_g_Tweight_allow_possessions-per-game', '20_game_avg_50_g_Tweight_allow_possessions-per-game', '30_game_avg_25_g_HAweight_allow_possessions-per-game', '30_game_avg_5_g_Tweight_allow_possessions-per-game'],
                        'model': Pipeline([('scale',RobustScaler()), ('reg',Lasso(random_state = 1108, alpha = 0.001))]),
                        },
                    },
                'full-team':{
                    'linsvm': {
                        ...
                        'model': Pipeline([('scale',MinMaxScaler()), ('reg',LinearSVR(random_state = 1108, C = 2.45976772084, epsilon=0))]),
                        }, 
                    'ridge': {
                        ...
                        'model': Pipeline([('scale',MinMaxScaler()), ('reg',Ridge(random_state = 1108, solver = 'lsqr', alpha = 6.07138889187))]),
                        },  
                    'lasso': {
                        'features': ['expected_effective-field-goal-pct_allowed', '75_g_HAspread_allow_floor-percentage', 'expected_turnovers-per-possession_allowed', 'expected_effective-field-goal-pct_allowed', 'expected_offensive-rebounding-pct_allowed', '75_g_HAspread_allow_defensive-efficiency', '100_g_HAspread_for_defensive-efficiency', '100_g_HAspread_allow_assist--per--turnover-ratio', '50_g_HAspread_for_assist--per--turnover-ratio', '30_g_HAspread_for_offensive-efficiency', '100_g_HAspread_for_points-per-game'],
                        'model': Pipeline([('scale',RobustScaler()), ('reg',Lasso(random_state = 1108, alpha = 0.00125925622966, max_iter = 2000))]),
                        },
                    'lightgbm': {
                        ...
                        'model': Pipeline([('scale',StandardScaler()), ('reg', lgb.LGBMRegressor(random_state = 1108, n_estimators = 1400, colsample_bytree = 0.99323664013058, min_child_samples = 61, num_leaves = 46, subsample = 0.464276892923476, max_bin = 1557, learning_rate = 0.0028125))]),
                        }                            
                    }
                },
        'pts_scored': {
                'all':{
                    'linsvm': {
                        ...
                        'model': Pipeline([('scale',MinMaxScaler()), ('reg',LinearSVR(random_state = 1108, C = 1.19681838901, epsilon=0))]),
                        },
                    'ridge': {
                        'features': ['expected_ppp_for', '25_g_HAspread_for_points-per-game', '30_g_HAspread_for_defensive-efficiency', '100_g_HAspread_allow_block-pct', '30_g_HAspread_allow_floor-percentage', '100_g_HAspread_allow_assist--per--turnover-ratio', 'pregame_pts_pg_for', '75_g_HAspread_for_floor-percentage', '50_g_HAspread_for_points-per-game', '100_g_HAspread_for_floor-percentage', 'expected_offensive-rebounding-pct_for', 'pregame_ppp_for', '75_g_HAspread_for_offensive-efficiency', 'expected_pts_pg_for', 'expected_ppp_for', '10_g_HAspread_allow_personal-fouls-per-possession', 'expected_ftm-per-100-possessions_for', 'expected_effective-field-goal-pct_for', '75_g_HAspread_allow_percent-of-points-from-3-pointers', 'expected_poss_pg_for', '50_game_avg_50_g_Tweight_for_personal-fouls-per-possession', '50_game_avg_30_g_Tweight_for_personal-fouls-per-possession', '50_game_avg_30_g_HAweight_allow_two-point-rate', '10_game_avg_5_g_HAweight_for_points-per-game`/`possessions-per-game', '1_game_avg_50_g_HAweight_for_possessions-per-game', 'expected_turnovers-per-possession_for'],
                        'model': Pipeline([('scale',StandardScaler()), ('reg',Ridge(random_state = 1108, solver = 'lsqr', alpha = 0.00115411795019))]),
                        },                                              
                    },
                'possessions':{
                    'linsvm': {
                        ...
                        'model': Pipeline([('scale',MinMaxScaler()), ('reg',LinearSVR(random_state = 1108, C = 2.7208908100107254, epsilon=0))]),
                        },
                    'lightgbm': {
                        ...
                        'model': Pipeline([('scale',MinMaxScaler()), ('reg', lgb.LGBMRegressor(random_state = 1108, n_estimators = 2800, colsample_bytree = 0.961267241448647, min_child_samples = 9, num_leaves = 18, subsample = 0.596425797228693, max_bin = 1844, learning_rate = 0.0025))]),
                        },
                    'lasso': {
                        'features': ['expected_poss_pg_for', 'pregame_poss_pg_for', '10_game_avg_10_g_Tweight_for_possessions-per-game', '10_game_avg_10_g_HAweight_for_possessions-per-game', '25_g_HAspread_for_possessions-per-game', '50_g_HAspread_for_possessions-per-game', '1_game_avg_50_g_Tweight_for_possessions-per-game', '1_game_avg_5_g_Tweight_for_possessions-per-game', '20_game_avg_10_g_HAweight_for_possessions-per-game', '30_game_avg_25_g_Tweight_for_possessions-per-game', '30_game_avg_10_g_Tweight_for_possessions-per-game', '30_game_avg_5_g_Tweight_for_possessions-per-game', '30_game_avg_50_g_HAweight_for_possessions-per-game'],
                        'model': Pipeline([('scale',StandardScaler()), ('reg',Lasso(random_state = 1108, alpha = 0.0015540696172751227, max_iter = 2000))]),
                        },
                    },
                'target':{
                    'linsvm': {
                        ...
                        'model': Pipeline([('scale',MinMaxScaler()), ('reg',LinearSVR(random_state = 1108, C = 0.12071774068337349, epsilon=0))]),
                        },
                    'lightgbm': {
                        ...
                        'model': Pipeline([('scale',MinMaxScaler()), ('reg', lgb.LGBMRegressor(random_state = 1108, n_estimators = 500, colsample_bytree = 0.780673078959247, min_child_samples = 5, num_leaves = 17, subsample = 0.607386007072246, max_bin = 1307, learning_rate = 0.0075))]),
                        },
                    'ridge': {
                        'features': ['expected_ppp_for', 'pregame_ppp_for', '50_g_HAspread_for_points-per-game`/`possessions-per-game', 'expected_pts_pg_for', '50_g_HAspread_for_points-per-game', 'pregame_pts_pg_for', '25_g_HAspread_for_points-per-game`/`possessions-per-game', '20_game_avg_50_g_HAweight_for_points-per-game`/`possessions-per-game', '10_game_avg_5_g_HAweight_for_points-per-game`/`possessions-per-game', '10_game_avg_5_g_Tweight_for_points-per-game`/`possessions-per-game', '5_game_avg_50_g_HAweight_for_points-per-game`/`possessions-per-game'],
                        'model': Pipeline([('scale',MinMaxScaler()), ('reg',Ridge(random_state = 1108, solver = 'sparse_cg', alpha = 0.5619646264922706))]),
                        },
                    'lasso': {
                        ...
                        'model': Pipeline([('scale',StandardScaler()), ('reg',Lasso(random_state = 1108, alpha = 0.13526474650418807, max_iter = 2000))]),
                        },
                    },
                'offensive_stats': {                       
                    'linsvm': {
                        ...
                        'model': Pipeline([('scale',RobustScaler()), ('reg',LinearSVR(random_state = 1108, C = 0.0869486130678, epsilon=0))]),
                        },                          
                    'lightgbm': {
                        ...
                        'model': Pipeline([('scale',MinMaxScaler()), ('reg', lgb.LGBMRegressor(random_state = 1108, n_estimators = 500, colsample_bytree = 0.808603278021336, min_child_samples = 176, num_leaves = 24, subsample = 0.678375514083654, max_bin = 1032, learning_rate = 0.01))]),
                        }
                    }
                }
            }